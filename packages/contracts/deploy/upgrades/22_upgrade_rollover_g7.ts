...
const deployFn: DeployFunction = async (hre) => {
  hre.log('----------')
  hre.log('')
  hre.log('FlashRolloverLoan G7: Proposing upgrade...')

  const networkName = hre.network.name

  const flashRolloverLoan = await hre.contracts.get('FlashRolloverLoanWidget')
  ...
  await hre.upgrades.proposeBatchTimelock({
    ...
    description: ` 
# FlashRolloverLoan G7 (Extensions Upgrade)

* Upgrades the flash rollover loan contract to support SCF calculations.
`,
    _steps: [
      {
        proxy: flashRolloverLoan,
        implFactory: await hre.ethers.getContractFactory('FlashRolloverLoanWidget'),
...
// tags and deployment
deployFn.id = 'lender-commitment-forwarder:extensions:flash-rollover:g7-upgrade'
deployFn.tags = [
  'proposal',
  'upgrade',
  ...
  'lender-commitment-forwarder:extensions:flash-rollover:g7-upgrade',
]
